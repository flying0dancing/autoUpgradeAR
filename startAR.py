        ...
        for key,value in confi.items():
            if re.search('applicationServers',key,re.IGNORECASE):
                if value[0].has_key('installfolder') and value[0]['installfolder'] is not None and re.search('^/',value[0]['installfolder'],re.IGNORECASE):
                    ...
                    if returnVal==0:
                        slog=time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time()))+' start Agile REPORTER pass\n'
                        writeLog(cmdFolder+installproductsLog,'a',slog)
                        #copy log from host
                        if os.path.isfile(cmdFolder+prefixRemoteStartAR+'.log'):
                            os.remove(cmdFolder+prefixRemoteStartAR+'.log')
                        slog=time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time()))+' copy '+prefixRemoteStartAR+'.log'+' from host.\n'
                        writeLog(cmdFolder+installproductsLog,'a',slog)
                        os.system(r'pscp.exe -batch '+userinfo+host+':'+value[0]['installfolder']+'/'+prefixRemoteStartAR+r'.log .\\log\\')
                    else:
                        slog=time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time()))+' start Agile REPORTER fail\n'
                        writeLog(cmdFolder+installproductsLog,'a',slog)       
                else:#local install
